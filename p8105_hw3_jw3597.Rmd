...
brfss_smart2010 %>%
  group_by(state, year = 2002) %>%
  count(state)
  count(year = '2002', state) %>%
  table(year = 2002, state)
  count(state)
  count(state, year = 2002)
  group_by(year, state) %>%
  summarize(state = ) %>%
  tail

# I am making a "spaghetti plot" with the number of observations in each state from 2002 to 2010

  ggplot(brfss_smart2010, aes(x = , y = , color = )) + 
...
brfss_smart2010 %>%
  ...
  summarize(
    count(state)) %>%
  gplot(aes(x = year, y = nobs, color = state)) + 
...
* There are 134 aisles and the aisles with the most ordered items is 83 (most), 24, 123, and 120.
