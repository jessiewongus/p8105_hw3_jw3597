...
ggplot(gg2002_2010, aes(x = year, y = n, colour = state)) + geom_line(se = FALSE)
```

* Make a table showing, for the years 2002, 2006, and 2010, the mean and standard deviation of the proportion of “Excellent” responses across locations in NY State.
```{r}
table1 = brfss_smart2010 %>%
  filter(state == "NY", health_response == "Excellent") %>%
...
panel_plot = brfss_smart2010 %>%
  group_by(state, year) %>%
  mutate(excellent = sum((health_response == "Excellent")/(sample_size)), very_good = sum((health_response == "Very good")/(sample_size)), good = sum((health_response == "Good")/(sample_size)), fair = sum((health_response == "Fair")/(sample_size)), poor = sum((health_response == "Poor")/(sample_size))) %>%
  ggplot(panel_plot, aes(x = year, fill = observation)) + 
  geom_point(alpha = .5) +
...
* There are 134 aisles and the aisles with the most ordered items is 83 "fresh vegetables" (most), 24 "fresh fruits", 123 "packaged vegetable fruits", and 120 "yogurt".
